<?php
/* phpFastCache Class Available in Github */
...
class phpFastCache
{
    ...
    public static $debugging = true; // turn true for debugging
    ...
    public static function systemInfo() {
        ...
        if(count(self::$sys) == 0 ) {
            ...
            if(self::$sys[Cmethod()] == "disk" && !is_writable(self::getPath())) {
...
allow from 127.0.0.1";
    ...
    public static function autoconfig($name = "") {
        ...
        if($cache != "" && $cache != Cmethod() && $cache!="auto") {
            return $cache;
        }
        ...
        $os = self::getOS();
        // self::startDebug(Cmethod(),"User Set",__LINE__,__FUNCTION__);
        if(Cmethod() == "" || Cmethod() == "auto") {
            // self::startDebug(Cmethod(),"User Set Auto",__LINE__,__FUNCTION__);
            ...
            if(in_array(Cmethod(),array("files","pdo","mpdo"))) {
                self::$sys[Cmethod()] = "disk";
            }elseif(in_array(Cmethod(),array("apc","memcache","memcached","wincache","xcache"))) {
                self::$sys[Cmethod()] = "memory";
            } else {
                self::$sys[Cmethod()] = "";
            }

            if(self::$sys[Cmethod()] == "" || !in_array(Cmethod(),self::$supported_api)) {
                die("Don't have this Cache ".Cmethod()." In your System! Please double check!");
            }

            self::$sys['method'] = strtolower(Cmethod());
    ...
    private static function db($option = array()) {
        ...
        // debuging
        // self::startDebug(Cmethod(),"Check Storage",__LINE__,__FUNCTION__);
        $initDB = false;
